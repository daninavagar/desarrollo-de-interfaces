				...
				public void actionPerformed(ActionEvent e) {
					...
					if (BarraProgreso.getValue() == 100) {
						if (file_class.fileChecks() || !file_class.passwdchecks()) {
							JOptionPane.showMessageDialog(null, "! Vaya � Algo ha pasado, seguramente han fallado los archivos o las constrse�as estan mal");
							tiempo.stop();
							frame.dispose();
						}
	...
	private void initialize() throws IOException {
		...
		Boton_Login.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!checkLogin()) {
					JOptionPane.showInternalMessageDialog(null, "Login/Contrase�a Incorrecta");
				}  else {
					readConfigFile();
